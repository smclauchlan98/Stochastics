...
eqn2 = eps * eps * (r.^2 .* sin(theta) .* ([zeros(1, 1 / k); q2val(1:(1 / k) - 2, :)] ...
    + [q2val(2:(1 / k) - 1, :); q_BC(2, :)] ...
    - 2.0 * q2val) / (k^2) ... % Second r deriv
    + sin(theta) .* ([q2val(:, 2:(1 / k)) zeros((1 / k) - 1, 1)] ...
    + [zeros((1 / k) - 1, 1) q2val(:, 1:(1 / k) - 1)] ...
    - 2.0 * q2val) / (k^2) ... % Second theta deriv
    + 2.0 * sin(theta) .* ([q2val(2:(1 / k) - 1, :); q_BC(2, :)] ...
    - [zeros(1, 1 / k); q2val(1:(1 / k) - 2, :)]) / (2.0 * k) ... % First r deriv
    + cos(theta) ...
    .* ([q2val(:, 2:(1 / k)) zeros((1 / k) - 1, 1)] ...
    - [zeros((1 / k) - 1, 1) q2val(:, 1:(1 / k) - 1)]) / (2.0 * k)) ... % First theta deriv
    ...
    + 9.0 * q2val .* q3val.^4 / 4.0))) .* sin(theta);
