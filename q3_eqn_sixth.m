function eqn3 = q3_eqn_sixth(q1val, q2val, q3val, r, theta, t, eps, d, e, f, q_BC, k)

eqn3 = eps * eps * (r.^2 .* sin(theta) .* ([zeros(1, 1 / k); q3val(1:(1 / k) - 2, :)] ...
    + [q3val(2:(1 / k) - 1, :); q_BC(3, :)] ...
    - 2.0 * q3val) / (k^2) ... % Second r deriv
    + sin(theta) .* ([q3val(:, 2:(1 / k)) zeros((1 / k) - 1, 1)] ...
    + [zeros((1 / k) - 1, 1) q3val(:, 1:(1 / k) - 1)] ...
    - 2.0 * q3val) / (k^2) ... % Second theta deriv
    + 2.0 * sin(theta) .* ([q3val(2:(1 / k) - 1, :); q_BC(1, :)] ...
    - [zeros(1, 1 / k); q3val(1:(1 / k) - 2, :)]) / (2.0 * k) ... % First r deriv
    + cos(theta) ...
    .* ([q3val(:, 2:(1 / k)) zeros((1 / k) - 1, 1)] ...
    - [zeros((1 / k) - 1, 1) q3val(:, 1:(1 / k) - 1)]) / (2.0 * k)) ... % First theta deriv
    ...
    + 9.0 * q2val.^2 .* q3val.^3 / 2.0))) .* sin(theta);

% eqn3(:, 1) = zeros((1 / k) - 1, 1);
% eqn3(:, end) = zeros((1 / k) - 1, 1);
