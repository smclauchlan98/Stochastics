...
set(gcf,'visible','off')
ylim([0 1])
plotname = sprintf(['./Output/Sixth/Plots' ...
...
for p = 1:3
    ...
    for w = 1:MC
        ...
        for i = 1:n_sim
            ...
            tt = 0.0;
            while tt < T
                fprintf('Simulation %d of %d, alpha = %.2f, iteration %d of %d\n', ...
                    i, n_sim, alpha(p), it, vpa(T / dtref));
            ...
            avg_profile = sum(SPDEsol, 2) / size(SPDEsol, 1);
            min_profile(i, 2) = min(avg_profile);
        ...
        filename = sprintf(['./Output/Sixth/Solutions' ...
            '/min_values_t%.1feps%.1fT%dalpha%.2fnsim%d.csv'], ...
        ...
        filename = sprintf(['./error/sixth1D_t%.1feps%.1f' ...
            'T%dalpha%.2fMC%dnsim%d.csv'], ...
            t, eps, vpa(T), alpha(p), MC, n_sim);
